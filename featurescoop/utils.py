...
def get_scene_info(images, id=0, max_duration=0, frames_per_scene=0):
  ...
  if frames_per_scene == 0 or end_id == start_id:
    ...
    x = { int(np.interp(i,[start_id,end_id],[0,frames_per_scene])):i for i in range(start_id,end_id) }
...
def video_convert_video(video, filename):
  '''
  Convert video to a standard format
  '''
  # BIG problem here
  dest_dir = tempfile.mkdtemp()
  dest = os.path.join(dest_dir, filename + ".mp4")
  # Switch to libx264
  args = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-i", video, "-b:a", "128k", "-b:v", "2M", "-c:a", "copy", "-c:v", "libx264", dest]
...
def video_extract_frames(video, imgdir, fps=1):
  ...
  args = ["ffprobe", "-hide_banner", "-loglevel", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", video]
  duration = float(subprocess.check_output(args))
  print('Duration %.2f sec, fps %.2f, extracting %d frames' % (duration, fps, (duration*fps+1)))
  ...
  args = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-t", str(math.floor(duration)), "-i", video, "-vsync", "drop", "-vf", "fps="+str(fps)+",scale=320:240:force_original_aspect_ratio=decrease,pad=320:240:(ow-iw)/2:(oh-ih)/2", "-q:v", "10", os.path.join(imgdir, '%05d.jpg')]
...
def video_extract_scenes(video, threshold=0.3):
  ...
  '''
  args = ["ffprobe", "-hide_banner", "-show_frames", "-loglevel", "error", "-of", "csv", "-f", "lavfi", "movie=" + video + ",select='gt(scene,"+str(threshold)+")'"]
  ...
  # print(scenes)
  print('%d Scenes extracted, average scene length %.2f s' % (len(scenes)-1, duration/float(len(scenes)-1)))
  return scenes
...
def features_get_closest_image(features, id=0, feature=[], num_neighbors=5):
  ...
  id_closest = sorted(range(len(distances)), key=lambda k: distances[k])[1:num_neighbors + 1]
  return id_closest
...
def features_apply_pca(features, components_count=300, pca='', chunk_size=500, batch_size=32):
  '''
...
def graph_get_shortest_paths(graph, id1, id2):
  ...
  '''
  paths = graph.get_shortest_paths(id1, to=id2, mode=1, output='vpath', weights='weight')
  for path in paths:
    if not '' in path:
      return path
