...
def retrievePath(num_neighbors, start_id, order):
    ...
    if order == "StartToEnd":
        ...
        if start_id == -1:
            img1_feat = pca_features_start[
                int(len(pca_features_start) * random.random())
            ]
        else:
...
def sbuildScenesJson(img_ids, duration):
    ...
    scenes_timing = [
        get_scene_info(images, id, max_duration=duration)[0:2] for id in img_ids
    ]
    scenes_ids = [
        get_scene_info(images, id, max_duration=duration)[3:5] for id in img_ids
    ]
    ...
    descriptions = [
        pickle.load(
            open(os.path.join(str(get_info(images, id)[2]), DESCRIPTION_FN), "rb")
        )
        if os.path.isfile(os.path.join(str(get_info(images, id)[2]), DESCRIPTION_FN))
        else ""
        for id in img_ids
    ]
    ...
    scenes_details = list(
        dict.fromkeys(zip(videos, scenes_timing, scenes_ids, descriptions))
    )
    ...
    for s in scenes_details:
        ...
        a["poem"] = (
            lines[
                index.get_nns_by_vector(meanvector(s[3]), n=5)[int(random.random() * 5)]
            ]
            if s[3] != ""
            else ""
        )
...
class Getpath(Resource):
    def get(self):
        ...
        duration = (
            float(request.args.get("duration")) if request.args.get("duration") else 5.0
        )
        ...
        start_id = (
            int(request.args.get("start_id")) if request.args.get("start_id") else -1
        )

        min_length = (
            int(request.args.get("min_length")) if request.args.get("min_length") else 2
        )
...
class GetSimilarVideos(Resource):
    def get(self):
        ...
        id = int(request.args.get("id")) if request.args.get("id") else -1
        ...
        img_ids = features_get_closest_image(
            pca_features, id=id, num_neighbors=num_neighbors
        )
