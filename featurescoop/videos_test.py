...
def test_closest(pca_features, images):
    ...
    id_closest = features_get_closest_image(
        pca_features, id=query_img_id, num_neighbors=10
    )
...
def test_path(
    pca_features, images, pca_fp, classifier, graph, DATA_PATH, img1="", img2=""
):
    if img1 and img2:
        # Extract Features
        ...
        img1_closest_id = features_get_closest_image(
            pca_features, feature=imgs_pca_feat[0], num_neighbors=10
        )[int(10 * random.random())]
        img2_closest_id = features_get_closest_image(
            pca_features, feature=imgs_pca_feat[1], num_neighbors=10
        )[int(10 * random.random())]
    else:
...
def main(args):
    ...
    print(
        "Loaded %d pca_features of %d dimensions"
