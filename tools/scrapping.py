...
def main(args):
    ...
    if args.source == "instagram":
        ...
        for hashtag in hashtags:
            loader.dirname_pattern = os.path.join(
                args.destination, args.source, "#" + hashtag
            )
            posts = loader.download_hashtag(
                hashtag, max_count=args.count, post_filter=post_filter
            )
        ...
        for search_term in search_terms:
            ydl_opts["outtmpl"] = os.path.join(
                args.destination, args.source, search_term, "%(id)s.%(ext)s"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--count", type=int, help="Number of posts to download", default=50
    )
    parser.add_argument(
        "-s",
        "--source",
        type=str,
        help="Source to scrap",
        default="instagram",
